...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    const { name, category, price, imageUrl, remark } = body;
    ...
    const dish = await Dish.create({ name, category, price, imageUrl, remark });
    return sendJson(res, 200, { success: true, data: dish });
  }

  if (req.method === "PUT") {
    const { id, name, category, price, imageUrl, remark, status } = body;
